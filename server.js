...
app.get('/download/nhentai', async function(req, res, next) {
    ...
    res.writeHead(200, {
            ...
            'Content-disposition': `attachment; filename=${num}-${title}-nhentai.zip`,
...
app.get('/download/asmhentai', async function(req, res, next) {
    ...
    res.writeHead(200, {
            'Content-Type': 'application/zip',
            'Content-disposition': `attachment; filename=${num}-${title}-asmhentai.zip`,
            //'access-control-allow-origin': 'https://hitomi.la'
            //'Sec-Fetch-Mode': 'cors'
    });
    var zip = archiver('zip', {
    ...
    while (now <= end) {
        download_photo("https://images.asmhentai.com/^00([1-9]\d*|90*)$/"+url+"/"+now+".", now, 0, function(url, name, type, cnt) {
            if (cnt <= 4) {
                var stream = request(url + type);
                zip.append(stream, {name: path.join(`${title}(${num})`, `${name}.${type}`)});
            }
...
app.get('/download/h2r', async function(req, res, next) {
    ...
    res.writeHead(200, {
            'Content-Type': 'application/zip',
            'Content-disposition': `attachment; filename=${num}-${title}-hentai2read.zip`,
            //'access-control-allow-origin': 'https://hitomi.la'
            //'Sec-Fetch-Mode': 'cors'
    });
    var zip = archiver('zip', {
